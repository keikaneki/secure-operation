...
$(function(){
	...
	if(urlParam){
		function addParam(){
			$.each($("a"), function(i,elem){
				...
				if(!$(elem).data("paramed")){
					...
					//リンク先がsecureincなら
					if(href.includes("secureinc") || href.startsWith("/") || href.startsWith(".")){
		...
		for(i=0;i<param.length;i++){
			if(param[i].indexOf("utm_source")>=0 || param[i].indexOf("fbclid")>=0 || param[i].indexOf("gclid")>=0 || param[i].indexOf("testparam")>=0 ){
				paramNeed += "&" + param[i];
			}
		}
		if(paramNeed != ""){
	...
	$(".js-filter").on("click",function(e){
		...
		$(".js-filter-biz").parent().removeClass("active");
		$(this).parent().addClass("active");
		var i = $(this).attr("data-filter");
		if(i=="all"){
			$(".case-list .col").fadeIn();
		}else{
			var hideclass=".case-list .col:not(." + i + ")";
			var showclass=".case-list .col." + i;
			$(showclass).fadeIn();
			$(hideclass).fadeOut();
		}
	});
	$(".js-filter-biz").on("click",function(e){
		$(".js-filter").parent().removeClass("active");
		$(".js-filter-biz").parent().removeClass("active");
		$(this).parent().addClass("active");
		var i = $(this).attr("data-filter");
		if(i=="all"){
			...
			$(".case-list .col").fadeOut();
			$(".case-list .col").each(function(index){
				$(this).find(".biz").each(function(indiex2){
					var biz = $(this).text();
					if(i == biz){
						$(this).parents(".col").fadeIn();
					}
				});
			});
		}
