...
<!-- End Google Tag Manager -->
	...
	<title>受付完了｜お問い合わせ｜SECURE AI Office Base：社員を守り、事業も継続！オフィスはクラウドで管理する</title>
	...
	if (isset($_SESSION['NAME1_B'])) {
		...
		/*switch ($num) {
			...
			case 1:
				...
				$subj = "【AI Office Base】 よりお問い合わせがありました";
				$thxmes = "お問い合わせありがとうございました。\n";
...
//		$mail->AddCC($mailCcAddress);
		$mail->Body		= mb_convert_encoding(
					"ウェブサイトにて以下内容のお問い合わせを受け付けました。\n".
					"送信者には、折り返しご連絡差し上げる旨と、ご案内資料のURLを記載した自動返信メールが送られています。\n\n".
		...
		if ( $mail->send() ) {
			$mail->ClearAddresses();
			$mail->FromName		= mb_encode_mimeheader($fromUserName,"ISO-2022-JP","UTF-8"); // "表示名" <メールアドレス>
			$mail->Subject		= mb_encode_mimeheader("【AI Office Base】 お問い合わせありがとうございます","ISO-2022-JP", "UTF-8");
...
?>
				ご入力いただきましたメールアドレスに、<br>
				問い合わせ完了メールを送信いたしました。<br>
				担当者より折り返しご連絡を差し上げますので、今しばらくお待ちください。<br>
				<br>
				完了メールが届かない場合、処理が正常に行われていない可能性があります。<br>
				大変お手数ですが、再度お問い合わせフォームよりお問い合わせいただくか、<br>
				お急ぎの場合はお電話でお問い合わせいただくようお願い致します。
...
?>
	...
	<section class="formsection">
		<article class="mainsection">
			...
			お手数ですが再度フォームから情報を入力して送信してください。<br><br><a style="text-decoration:underline;" href="./index.php#form_b">お問い合わせフォームに戻る</a></p>
...
?>
	...
	<section class="formsection">
		<article class="mainsection">
			...
			<p>お手数ですが再度フォームから情報を入力して送信してください。<br><br><a style="text-decoration:underline;" href="./index.php#form_b">お問い合わせフォームに戻る</a></p>
